...
class Network:
    ...
    def compute_vehicle_to_order_distance(self, vehicle_location: VehicleLocation, order_location: OrderLocation) -> float:
        ...
        if math.isinf(vehicle_to_order_distance):
            return 0;
    ...
    def is_smaller_bound_distance(distance: float, bound_distance: float):
        ...
        """
        # print(distance)
        if math.isinf(distance) or math.isinf(bound_distance) or math.isnan(distance) or math.isnan(bound_distance):
            return is_enough_small(round(0), round(0+ DISTANCE_EPS))
