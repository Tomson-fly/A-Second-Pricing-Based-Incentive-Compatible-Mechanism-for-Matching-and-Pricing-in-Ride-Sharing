...
class BaseGraph:
    ...
    def generate_random_locations(self, location_number: int, location_type) -> List[
        Union[OrderLocation, VehicleLocation]]:
        raise NotImplementedError

    def move_to_target_index(self, vehicle_location: VehicleLocation, target_index: int, could_drive_distance: float,
                             is_random_drive=True) -> float:
        """
        ...
        real_drive_distance = self.get_shortest_distance_by_osm_index(vehicle_location.osm_index,
                                                                      target_index)  # 实际行驶的距离
        if is_enough_small(real_drive_distance - could_drive_distance, DISTANCE_EPS):  # 可以到target_index的情况
            ...
            while not is_enough_small(could_drive_distance, DISTANCE_EPS):  # 需要行驶的距离已经过小了的情况
                ...
                vehicle_to_next_distance = self.get_shortest_distance_by_osm_index(vehicle_location.osm_index,
                                                                                   next_index)  # 因为只是移动一个格子, 所以只是移动一个小格子
...
class RoadGraph(BaseGraph):
    ...
    raw_graph: 真实的图
    """
    __slots__ = ["_shortest_distance", "_shortest_path", "_access_osm_index", "_adjacent_location_osm_index",
                 "_adjacent_location_driven_distance", "_adjacent_location_goal_index", "_index_set"]
    ...
    def can_move_to_goal_index(self, vehicle_location: VehicleLocation, order_location: OrderLocation) -> bool:
        ...
        if reverse_distance_c == np.inf or (
                distance_b != np.inf and is_enough_small(distance_c - vehicle_location.driven_distance + distance_b,
                                                         distance_a + vehicle_location.driven_distance)):
    ...
    def move_to_random_index(self, vehicle_location: VehicleLocation, could_drive_distance: float) -> float:
        ...
        if vehicle_location.is_between:  # 车辆处于两个节点之间
            # 首先车辆行驶两节点之间的距离
            vehicle_to_goal_distance = self.get_shortest_distance_by_osm_index(vehicle_location.osm_index,
                                                                               vehicle_location.goal_index) - vehicle_location.driven_distance
...
class GridGraph(BaseGraph):
    ...
    def _get_best_next_xy(self, now_x: int, now_y: int, target_x: int, target_y: int) -> Tuple[int, int]:
        ...
        :return:
        """
        next_xy_lists = [(now_x + direction[0], now_y + direction[1]) for direction in self._directions if
                         0 <= now_x + direction[0] <= self._graph_size and 0 <= now_y + direction[
                             1] <= self._graph_size]
        next_xy_lists = [(next_x, next_y, self._get_shortest_distance_by_xy(next_x, next_y, target_x, target_y)) for
                         next_x, next_y in next_xy_lists]
    ...
    def can_move_to_goal_index(self, vehicle_location: VehicleLocation, order_location: OrderLocation) -> bool:
        ...
        这个函数不可以修改 vehicle_location的值
        """
        distance_a = self.get_shortest_distance_by_osm_index(vehicle_location.osm_index,
                                                             order_location.osm_index)  # 意义看文档
        distance_b = self.get_shortest_distance_by_osm_index(vehicle_location.goal_index,
                                                             order_location.osm_index)  # 意义看文档
        distance_c = self.get_shortest_distance_by_osm_index(vehicle_location.osm_index,
                                                             vehicle_location.goal_index)  # 意义看文档
        # 用于判断车是否从goal_index到order_location
        diff_distance = (distance_c - vehicle_location.driven_distance + distance_b) - (
                    distance_a + vehicle_location.driven_distance)
